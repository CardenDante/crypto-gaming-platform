...
// Maximum length for image URL based on DB constraints
...
export default function AdminPromotions() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Validate image URL length if this is the image URL field
    if (name === 'imageUrl' && value && value.length > MAX_IMAGE_URL_LENGTH) {
      setValidationErrors(prev => ({
        ...prev,
        imageUrl: `Image URL must be less than ${MAX_IMAGE_URL_LENGTH} characters (currently ${value.length} characters)`,
      }));
    }
  };
  ...
  const validateForm = () => {
    ...
    if (formData.imageUrl && formData.imageUrl.length > MAX_IMAGE_URL_LENGTH) {
      errors.imageUrl = `Image URL must be less than ${MAX_IMAGE_URL_LENGTH} characters`;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to ${editingPromotion ? 'update' : 'create'} promotion`);
      }
      ...
      setError(`Failed to ${editingPromotion ? 'update' : 'create'} promotion: ${err.message}`);
    } finally {
      setProcessing(false);
    }
  };
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Optional. Leave empty to use default promotion image. Max {MAX_IMAGE_URL_LENGTH} characters.
            </p>
            ...
            {formData.imageUrl && (
              <p className="mt-1 text-xs text-gray-500">
                Character count: {formData.imageUrl.length} / {MAX_IMAGE_URL_LENGTH}
              </p>
            )}
          </div>
