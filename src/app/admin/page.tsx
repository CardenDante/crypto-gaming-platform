...
export default function AdminDashboard() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      if (!depositsRes.ok) {
        ...
        throw new Error(`Failed to fetch deposits: ${depositsRes.status} ${errorText}`);
      }
      ...
      if (!withdrawalsRes.ok) {
        ...
        throw new Error(`Failed to fetch withdrawals: ${withdrawalsRes.status} ${errorText}`);
      }
  ...
  const handleUpdateTransaction = async () => {
    ...
    try {
      ...
      console.log(`Updating transaction ${selectedTransaction.id} to status ${updateStatus}`);
  ...
  const openTransactionModal = (transaction: Transaction, status: 'COMPLETED' | 'REJECTED' | null = null) => {
  ...
  return (
    <div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      {transaction.status === 'PENDING' && (
                        <>
                          <button
                            type="button"
                            onClick={() => openTransactionModal(transaction, 'COMPLETED')}
                          ...
                          <button
                            type="button"
                            onClick={() => openTransactionModal(transaction, 'REJECTED')}
      ...
      {/* Transaction Update Modal using React Portal */}
      <Modal
        isOpen={showUpdateModal}
        onClose={() => !processingTransaction && setShowUpdateModal(false)}
        title={updateStatus
          ? `${updateStatus === 'COMPLETED' ? 'Approve' : 'Reject'} ${
              selectedTransaction?.type.charAt(0).toUpperCase() + selectedTransaction?.type.slice(1)
            }`
          : `Transaction Details`}
        onSubmit={updateStatus ? handleUpdateTransaction : undefined}
        submitLabel={updateStatus === 'COMPLETED' ? 'Approve' : updateStatus === 'REJECTED' ? 'Reject' : undefined}
        submitColor={updateStatus === 'COMPLETED' ? 'green' : updateStatus === 'REJECTED' ? 'red' : 'indigo'}
